...
The data was accessed through the SODA Consumer API client on 17th November, 2018. Click [here](https://dev.socrata.com/consumers/getting-started.html) to read more about the API. 

The `tidyverse`, `Rsocrata` and `lubridate` libraries were required to connect with the API and subsequently manipulate and clean the data.
...
### Subsetting Data 
We were interested in 2017, 2016, 2015 and 2014 data specifically so we extracted crime information for those years. We were also interested in felony crimes so we excluded misdemeanors and violations from the dataset. 
...
The nyc_felony_crimes dataset has 588,704 observations and 31 variables. The subsetted sex_drug_weapons dataset has 46,692 observations and 15 variables. 

<br>

### Reverse Geocoding

The dataset comes without zipcode or neighborhood information; there are only geographic coordinates. This makes chloropleth mapping very difficut. We reverse geocoded the coordinates. 
...
The following list provides all 17 variables in the dataset with their descriptions:
...
`cmplnt_fr_dt`: exact end date of occurrent for the reported incident <br> <p>
