...
sex_crimes %>% group_by(prem_typ_desc) %>% summarize(n = n()) %>% View

x = read.delim("https://www2.census.gov/geo/docs/maps-data/data/rel/zcta_county_rel_10.txt", sep = "," ) 
```
...
```{r}
year = read_html("http://worldpopulationreview.com/us-cities/new-york-city-population/") %>% 
  html_nodes("td:nth-child(1)") %>% html_text()

population = read_html("http://worldpopulationreview.com/us-cities/new-york-city-population/") %>% 
  html_nodes("td:nth-child(2)") %>% html_text()

x = tibble(year = as.numeric(year),
           population = population) %>% drop_na() %>% 
  mutate(population = as.numeric(gsub("," , "", population)))

read_csv("https://data.cityofnewyork.us/api/views/kku6-nxdu/rows.csv?accessType=DOWNLOAD") %>% View

season_df %>% 
  summarize_all(funs(sum(is.na(.))))

filter_df %>% group_by(vic_race) %>% summarize(number = n()) %>% 
  ggplot(aes(x = vic_race, y = number)) + geom_col()
```
