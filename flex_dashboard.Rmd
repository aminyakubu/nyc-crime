...
nyc_pal <- colorNumeric("Greens", domain = map_json@data$number)

map_json %>% 
  leaflet() %>% 
  #addTiles() %>% 
  addProviderTiles("OpenStreetMap.BlackAndWhite", options = providerTileOptions(opacity = 0.1))%>% 
  ...
  # add labels that display mumber
   ...
   addLegend(pal = nyc_pal,
                   ...
                   position = "bottomright") %>% addControl("Weapon-Related, Sex-Related and Drug-Related Felony offenses 2017", position = "topright")
```
